...
class State(rx.State):
    ...
    def load_entries(self) -> list[Customer]:  # type: ignore
        ...
        with rx.session() as session:
                ...
                query = query.where(
                    or_(
                        *[
                            ...
                            if field not in ["quote_id", "total_amount"]
                        ],
            ...
            if self.sort_value:
                ...
                if self.sort_value == "total_amount":
    ...
    def get_current_month_values(self):
        ...
        current_month_users = [
            ...
            if datetime.strptime(user.created_at, "%Y-%m-%d %H:%M:%S") >= start_of_month
        ]
        num_customers = len(current_month_users)
        total_payments = sum(user.total_amountl for user in current_month_users)
        num_delivers = len(
